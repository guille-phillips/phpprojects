...
RULESTEXT;
	...
	<?php
		...
		function unbox1($element) {return is_array($element)?'('.$element[0].')':$element;}
		function unbox2($element) {return is_array($element)?'('.$element[0].')':$element;}
		function sort1($a,$b) {return strlen($a[0])<strlen($b[0])?1:-1;}
		function sort2($a,$b) {return strlen($a[1])<strlen($b[1])?1:-1;}
		...
		if (isset($_POST['submit'])) {
			if ($has_fixed_pattern) {
				...
				usort($left_words, 'sort1');
				
				usort($right_words, 'sort2');
